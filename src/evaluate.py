...
def evaluate_model(
    model, features, target, timestamps, model_name, dataset_name, plots_folder
):
    ...
    fig = preview_prediction(
        timestamps, target, predictions, f"{model_name} - {dataset_name}"
    )
...
def evaluate_model_on_datasets(
    model, model_name, historical_data, finetuning_data, plots_folder
):
...
def main() -> None:
    if len(sys.argv) != 4:
        ...
        print(
            "\tpython3 evaluate.py <model-folder> <dataset-historical-validation> <dataset-finetuning-validation>\n"
        )
        exit(1)
    ...
    if baseline_exists:
        ...
        improvement_metrics = calculate_improvement_metrics(
            (bl_hist_rmse, bl_ft_rmse), (ft_hist_rmse, ft_ft_rmse)
        )
    ...
    update_baseline = should_update_baseline(
        baseline_metrics, (ft_hist_rmse, ft_ft_rmse), baseline_exists
    )
